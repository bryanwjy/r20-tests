...
static constexpr __RXX tuple<int, float> t(73, 0.2f);
static_assert(__RXX get<0>(t) == 73, "");
static_assert(__RXX get<int>(t) == 73, "");
static_assert(__RXX ranges::get_element<0>(t) == 73, "");
...
concept gettable =
    requires(T&& val) { __RXX ranges::get_element<I>(std::forward<T>(val)); };

static_assert(!gettable<decltype((t)), 2>, "Should not be gettable");

static constexpr __RXX tuple<long long, double> u(t);
static constexpr __RXX tuple<int, short, char, long long> v(
    73, 2, 22, 2652547);
static constexpr __RXX tuple<int, short, char, long long, short> w(
    73, 2, 22, 2652547, 7231);
static constexpr __RXX tuple<int, float> t1(2, 0.354);
static constexpr __RXX tuple<long long, double> t2(2, 0.354);

static_assert(sizeof(decltype(__RXX tuple<int, float, long long, double>{
    std::declval<__RXX tuple<int const&, float const&, long long const&,
        double const&>>()})));
static constexpr __RXX tuple<int, float, long long, double> t3(
    __RXX tuple_cat(t1, t2));

static_assert(std::same_as<decltype(__RXX tuple_cat(t1, t2)),
    __RXX tuple<int, float, long long, double>>);

static_assert(
    __RXX get<char>(v) == 22, "Should be gettable by type if target exists");
static_assert(__RXX get<char>(w) == 22,
...
static_assert(
    std::is_same_v<decltype(__RXX forward_as_tuple(f<0>{}, f<1>{}, f<2>{})),
        __RXX tuple<f<0>&&, f<1>&&, f<2>&&>>,
    "__RXX forward_as_tuple must be lvalue if lvalue, rvalue otherwise");

static_assert(std::is_same_v<decltype(__RXX forward_as_tuple(
                                 std::declval<f<0>&>(), f<1>{}, f<2>{})),
                  __RXX tuple<f<0>&, f<1>&&, f<2>&&>>,
    "__RXX forward_as_tuple must be lvalue if lvalue, rvalue otherwise");

static_assert(
    std::is_same_v<decltype(__RXX forward_as_tuple(
                       std::declval<f<0> const&>(), std::declval<f<0>&>(),
                       std::declval<f<0> const volatile&>())),
        __RXX tuple<f<0> const&, f<0>&, f<0> const volatile&>>,
    "__RXX forward_as_tuple must conserve cv qualifiers");

static_assert(
    std::is_same_v<decltype(__RXX forward_as_tuple(std::declval<f<0> const>(),
                       std::declval<f<0>>(),
                       std::declval<f<0> const volatile>())),
        __RXX tuple<f<0> const&&, f<0>&&, f<0> const volatile&&>>,
    "__RXX forward_as_tuple must conserve cv qualifiers");
...
static_assert(std::is_same_v<decltype(__RXX forward_as<std::tuple>(
                                 f<0>{}, f<1>{}, f<2>{})),
                  std::tuple<f<0>&&, f<1>&&, f<2>&&>>,
    "__RXX forward_as<std::tuple> must be lvalue if lvalue, rvalue otherwise");
...
static_assert(
    std::is_same_v<decltype(__RXX forward_as<std::tuple>(
                       std::declval<f<0> const&>(), std::declval<f<0>&>(),
                       std::declval<f<0> const volatile&>())),
        std::tuple<f<0> const&, f<0>&, f<0> const volatile&>>,
    "__RXX forward_as<std::tuple> must conserve cv qualifiers");
...
static_assert(!__RXX is_explicit_constructible_v<
                  __RXX tuple<implicit_obj, int, float>>,
    "tuples containing non explicit constructible elements should not be "
    "explicitly constructible");
static_assert(
    __RXX is_explicit_constructible_v<__RXX tuple<explicit_obj, int, float>>,
...
static_assert(
    !std::is_copy_constructible_v<__RXX tuple<move_only<0>, copy_only<0>>>,
    "if non-copy_only element exists, tuple should not be copy_only");
static_assert(
    !std::is_copy_assignable_v<__RXX tuple<move_only<0>, copy_only<0>>>,
    "if non-copy_only element exists, tuple should not be copy_only");
static_assert(!std::is_copy_constructible_v<__RXX tuple<move_only<0>, int>>,
    "if non-copy_only element exists, tuple should not be copy_only");
static_assert(!std::is_copy_assignable_v<__RXX tuple<move_only<0>, int>>,
    "if non-copy_only element exists, tuple should not be copy_only");
static_assert(
    std::is_copy_constructible_v<__RXX tuple<copy_only<0>, copy_only<1>>>,
    "if all elements are copy_only, tuple should be copy_only");
static_assert(
    std::is_copy_assignable_v<__RXX tuple<copy_only<0>, copy_only<1>>>,
    "if all elements are copy_only, tuple should be copy_only");

static_assert(std::is_move_constructible_v<__RXX tuple<int, int>>,
    "fundamental tuple is move constructible");

static_assert(
    !std::is_constructible_v<__RXX tuple<move_only<0>, move_only<1>>,
        __RXX tuple<move_only<0>&&, move_only<1>&&> const&>,
    "Move only tuple should not be constructible with an lvalue tuple of "
    "rvalue references of the "
    "same element types");
static_assert(std::is_constructible_v<__RXX tuple<move_only<0>, move_only<1>>,
                  __RXX tuple<move_only<0>&&, move_only<1>&&>>,
    "Move only tuple should be constructible with an rvalue tuple of rvalue "
    "references of the same "
    "element types");

static_assert(std::is_constructible_v<__RXX tuple<copy_only<0>, copy_only<1>>,
                  __RXX tuple<copy_only<0>&&, copy_only<1>&&> const&>,
    "Copy only tuple should be constructible with an lvalue tuple of rvalue "
    "references of the same "
    "element types");

static_assert(
    std::is_move_constructible_v<__RXX tuple<copy_only<0>, copy_only<1>>>,
    "if non-move_only element exists, tuple should still be movable");

static_assert(
    std::is_move_constructible_v<__RXX tuple<move_only<0>, copy_only<0>>>,
    "if non-move_only element exists, tuple should still be movable");
static_assert(
    std::is_move_assignable_v<__RXX tuple<move_only<0>, copy_only<0>>>,
    "if non-move_only element exists, tuple should still be movable");
static_assert(
    std::is_move_assignable_v<__RXX tuple<copy_only<0>, copy_only<1>>>,
    "if non-move_only element exists, tuple should still be movable");

static_assert(
    std::is_move_constructible_v<__RXX tuple<move_only<0>, move_only<1>>>,
    "if all elements are move_only, tuple should be move_only");
static_assert(
    std::is_move_assignable_v<__RXX tuple<move_only<0>, move_only<1>>>,
    "if all elements are move_only, tuple should be move_only");
...
static_assert(
    std::is_default_constructible_v<__RXX tuple<defaultable<0>, int, float>>,
    "If all element are default constructible, tuple is default constructible");

static_assert(std::is_constructible_v<__RXX tuple<int, defaultable<0>, float>,
                  int, defaultable<0>, float>,
    "If all element are default constructible, tuple is default constructible");

static_assert(
    std::is_nothrow_default_constructible_v<
        __RXX tuple<defaultable<0>, defaultable<1>, defaultable<3>>>,
    "If all element are nothrow constructible, tuple is nothrow constructible");

static_assert(std::is_nothrow_constructible_v<__RXX tuple<long long, double>,
                  __RXX tuple<int, float>>,
    "If all element are nothrow constructible, tuple is nothrow constructible");

static_assert(std::is_constructible_v<__RXX tuple<long long, double>,
                  __RXX tuple<int, float>&>,
    "If all element are constructible, tuple is constructible from reference");
static_assert(std::is_constructible_v<__RXX tuple<long long, double>,
                  __RXX tuple<int, float> const&>,
...
static_assert(!std::is_default_constructible_v<
                  __RXX tuple<non_defaultable<0>, int, float>>,
