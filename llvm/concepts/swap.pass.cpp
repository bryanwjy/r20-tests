...
constexpr bool check_lvalue_adl_swappable() {
    ...
    ASSERT_NOEXCEPT(xranges::swap(x, y));
...
constexpr bool check_rvalue_adl_swappable() {
    ASSERT_NOEXCEPT(
        xranges::swap(rvalue_adl_swappable(0), rvalue_adl_swappable(1)));
...
constexpr bool check_lvalue_rvalue_adl_swappable() {
    ...
    ASSERT_NOEXCEPT(xranges::swap(x, lvalue_rvalue_adl_swappable(1)));
...
constexpr bool check_rvalue_lvalue_adl_swappable() {
    ...
    ASSERT_NOEXCEPT(xranges::swap(rvalue_lvalue_adl_swappable(1), x));
...
constexpr bool check_throwable_swappable() {
    ...
    ASSERT_NOT_NOEXCEPT(xranges::swap(x, y));
    assert(check_swap_21(x, y));
    return true;
}
...
auto global_x = 0;
ASSERT_NOEXCEPT(xranges::swap(global_x, global_x));
...
constexpr bool check_swappable_pointers() {
    char const* x = "hello";
    ASSERT_NOEXCEPT(xranges::swap(x, x));
...
void check_swap(expected<T> const& e) {
    ...
    static_assert(noexcept(xranges::swap(a, b)) == bool(is_noexcept));
}
