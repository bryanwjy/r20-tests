...
struct ThrowOnIncrementIterator {
    ...
    friend bool operator==(ThrowOnIncrementIterator const&,
        ThrowOnIncrementIterator const&) = default;
};

struct ThrowOnIncrementView : IntBufferView {
    ThrowOnIncrementIterator begin() const {
        return ThrowOnIncrementIterator{buffer_};
    }
    ThrowOnIncrementIterator end() const {
        return ThrowOnIncrementIterator{buffer_ + size_};
    }
};

// Cannot run the test at compile time because it is not allowed to throw
// exceptions
void test() {
    int buffer[] = {1, 2, 3};
    {
        // zip iterator should be able to be destroyed after member function
        // throws
        xranges::zip_view v{ThrowOnIncrementView{buffer}};
        auto it = v.begin();
        try {
            ++it;
            assert(false); // should not be reached as the above expression
                           // should throw.
        } catch (int e) {
            assert(e == 5);
        }
    ...
    {
        // zip iterator should be able to be assigned after member function
        // throws
        xranges::zip_view v{ThrowOnIncrementView{buffer}};
        auto it = v.begin();
        try {
