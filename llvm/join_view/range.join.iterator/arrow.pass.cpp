...
template <class T>
concept HasArrow = std::input_iterator<T> &&
    (std::is_pointer_v<T> || requires(T i) { i.operator->(); });
...
struct move_only_input_iter_with_arrow {
    ...
    constexpr move_only_input_iter_with_arrow(
        move_only_input_iter_with_arrow&&) = default;
    constexpr move_only_input_iter_with_arrow(
        move_only_input_iter_with_arrow const&) = delete;
...
struct move_iter_sentinel {
    ...
    constexpr bool operator==(
        move_only_input_iter_with_arrow<Base> const& other) const {
        return it_ == other.it_;
    }
};
static_assert(std::sentinel_for<move_iter_sentinel<int*>,
    move_only_input_iter_with_arrow<int*>>);

struct MoveOnlyIterInner :
    BufferView<move_only_input_iter_with_arrow<Box*>,
        move_iter_sentinel<Box*>> {
...
static_assert(xranges::input_range<MoveOnlyIterInner>);
...
struct arrow_input_iter {
    ...
    friend constexpr bool operator==(
        arrow_input_iter const& x, arrow_input_iter const& y) = default;
};

using ArrowInner = BufferView<arrow_input_iter<Box*>>;
static_assert(xranges::input_range<ArrowInner>);
static_assert(HasArrow<xranges::iterator_t<ArrowInner>>);
...
constexpr bool test() {
...

    {
        ...
        ValueView<Box> children[4] = {BoxView(buffer[0]), BoxView(buffer[1]),
            BoxView(buffer[2]), BoxView(buffer[3])};
        xranges::join_view jv(ValueView<ValueView<Box>>{children});
...

    {
        ...
        static_assert(HasArrow<decltype(xranges::begin(inners[0]))>);
        static_assert(!HasArrow<decltype(jv.begin())>);
    }

    {
        // LWG3500 `join_view::iterator::operator->()` is bogus
        // `operator->` should not be defined if inner iterator does not have
        // `operator->` !has-arrow<InnerIter> && copyable<InnerIter>
