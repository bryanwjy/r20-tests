...
static_assert(HasBegin<rxx::ranges::as_rvalue_view<SimpleView>>);
static_assert(HasBegin<const rxx::ranges::as_rvalue_view<SimpleView>>);
static_assert(HasBegin<rxx::ranges::as_rvalue_view<NonSimpleView>>);
static_assert(HasBegin<const rxx::ranges::as_rvalue_view<NonSimpleView>>);
static_assert(HasBegin<rxx::ranges::as_rvalue_view<NonConstView>>);
static_assert(!HasBegin<const rxx::ranges::as_rvalue_view<NonConstView>>);

template <class Iter, class Sent> constexpr void test_range() {
  int a[] = {1, 2};
...
template <class Iter, class Sent> constexpr void test_const_range() {
  int a[] = {1, 2};
  auto range = WrapRange{Iter(a), Sent(Iter(a + 2))};
  const rxx::ranges::as_rvalue_view view(std::views::all(range));
  std::same_as<std::move_iterator<Iter>> decltype(auto) iter = view.begin();
  assert(base(iter.base()) == std::begin(a));
}
