...
static_assert(HasEnd<rxx::ranges::as_rvalue_view<DefaultConstructibleView>>);
static_assert(
    HasEnd<const rxx::ranges::as_rvalue_view<DefaultConstructibleView>>);
static_assert(HasEnd<rxx::ranges::as_rvalue_view<NonConstView>>);
static_assert(!HasEnd<const rxx::ranges::as_rvalue_view<NonConstView>>);

static_assert(std::is_same_v<decltype(std::declval<rxx::ranges::as_rvalue_view<
                                          DefaultConstructibleView>>()
                                          .end()),
                             std::move_iterator<int *>>);
static_assert(
    std::is_same_v<decltype(std::declval<const rxx::ranges::as_rvalue_view<
                                NonConstCommonRange>>()
                                .end()),
                   std::move_sentinel<sentinel_wrapper<int *>>>);

template <class Iter, class Sent, bool is_common> constexpr void test_range() {
  ...
  rxx::ranges::as_rvalue_view view(std::move(range));
  std::same_as<Expected> decltype(auto) iter = view.end();
  assert(base(base(iter.base())) == std::end(a));
}

template <class Iter, class Sent> class WrapRange {
  Iter iter_;
  Sent sent_;

public:
  constexpr WrapRange(Iter iter, Sent sent)
      : iter_(std::move(iter)), sent_(std::move(sent)) {}
...
constexpr void test_const_range() {
  ...
  const rxx::ranges::as_rvalue_view view(std::move(range));
  std::same_as<Expected> decltype(auto) iter = view.end();
  assert(base(base(iter.base())) == std::end(a));
}
...
constexpr bool test() {
  ...
  {
    rxx::ranges::as_rvalue_view view(CVCallView{});
    (void)view.end();
  ...
  { // check that with a std::move_iterator begin() doesn't return
    ...
    rxx::ranges::as_rvalue_view view{move_iterator_view{}};
