...
using VRange =
    std::conditional_t<RefIsGlvalue, std::vector<Inner>, RvalueVector<Inner>>;
...
constexpr void test_pre_increment() {
    { // `V` and `Pattern` are not empty. Test return type too.
...

        {
            ...
            static_assert(!CanPreIncrement<Iter const>);
        ...
        if constexpr (RefIsGlvalue) {
            ...
            static_assert(!CanPreIncrement<CIter const>);
    ...
    { // Only first element of `V` is not empty. `Pattern` is not empty. `V`
      // models input range.
        using V = BasicView<VRange<RefIsGlvalue, std::string>, ViewProperties{},
            DefaultCtorInputIter>;
...
constexpr void test_post_increment() {
    { // `V` and `Pattern` are not empty. Return type should be `iterator`.
        ...
        static_assert(!CanPostIncrement<Iter const>);
        static_assert(CanPostIncrement<CIter>);
        static_assert(!CanPostIncrement<CIter const>);
    ...
    { // `Pattern` is empty, `V` is not. Value of `ref-is-glvalue` is false
    ...
    { // `V` has empty subrange in the middle, `Pattern` is not empty.
        // OuterIter does not model forward iterator (return type should be
        // `void`).
        using Inner = std::vector<int>;
        using V = BasicVectorView<Inner, ViewProperties{.common = false},
