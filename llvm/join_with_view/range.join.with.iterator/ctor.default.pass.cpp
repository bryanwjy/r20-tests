...
constexpr bool test() {
    ...
    { // `InnerIter` is not default constructible (does not model forward
      // iterator, JWV cannot be const-accessed)
        ...
        using V = BasicVectorView<Inner, ViewProperties{.common = false},
            forward_iterator>;
        using Pattern =
            BasicVectorView<char, ViewProperties{}, forward_iterator>;
        using JWV = xranges::join_with_view<V, Pattern>;
        using Iter = xranges::iterator_t<JWV>;
