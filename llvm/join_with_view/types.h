...
concept ConstructionIsExplicit =
    std::constructible_from<Tp, Args...> && !requires(Args&&... args) {
...
public:
    ...
    requires requires { xranges::to<Data>(std::forward<R>(r)); }
...
template <class Tp, ViewProperties Prop, template <class...> class It,
...
public:
    ...
    constexpr explicit RvalueVector(Vec vec)
        : range_(std::move(vec), AsPrvalue{}) {}
    constexpr explicit RvalueVector(std::initializer_list<Tp> il)
        : RvalueVector(Vec(il)) {}
...
public:
    ...
    constexpr bool operator==(DefaultCtorInputIter const&) const = default;
};
...
public:
    ...
    constexpr auto end() {
        return sentinel_wrapper(cpp20_input_iterator(range_.end()));
    }
    constexpr auto begin() const {
        return cpp20_output_iterator(range_.begin());
    }
    constexpr auto end() const {
        return sentinel_wrapper(cpp20_output_iterator(range_.end()));
    }
};

template <class Tp>
using ForwardViewButInputWhenConst = BasicVectorView<Tp,
    ViewProperties{.common = false}, forward_iterator, cpp20_input_iterator>;
...
public:
    ...
    bool operator==(ForwardIteratorWithInputCategory const&) const;
};

template <class It>
explicit ForwardIteratorWithInputCategory(It)
    -> ForwardIteratorWithInputCategory<It>;
...
public:
    ...
    friend constexpr It base(EqComparableInputIter const& i) { return i.it_; }

    friend constexpr bool operator==(
        EqComparableInputIter const& left, EqComparableInputIter const& right) {
...
namespace lwg4074 { // Helpers for LWG-4074 ("compatible-joinable-ranges is
                    // underconstrained")
...
namespace selftest {
using BV1 =
    BasicView<std::string, ViewProperties{.simple = true}, forward_iterator>;
static_assert(xranges::forward_range<BV1>);
static_assert(!xranges::bidirectional_range<BV1>);
static_assert(xranges::common_range<BV1>);
static_assert(simple_view<BV1>);

using BV2 = BasicView<RvalueVector<std::string>,
    ViewProperties{.simple = false, .common = false}, cpp20_input_iterator>;
...
static_assert(xranges::input_range<IRBOWC>);
static_assert(xranges::output_range<const IRBOWC&, int>);

using FVBIWC = ForwardViewButInputWhenConst<int>;
static_assert(std::default_initializable<FVBIWC>);
static_assert(xranges::view<FVBIWC>);
static_assert(xranges::forward_range<FVBIWC>);
static_assert(!xranges::common_range<FVBIWC>);
static_assert(xranges::input_range<const FVBIWC&>);
static_assert(!xranges::forward_range<const FVBIWC&>);
static_assert(!xranges::common_range<const FVBIWC&>);
...
static_assert(std::same_as<FIWIC::iterator_category,
    std::iterator_traits<FIWIC>::iterator_category>);
...
static_assert(xranges::view<COV>);
static_assert(xranges::range<const COV>);
static_assert(!xranges::common_range<COV>);
static_assert(!xranges::common_range<const COV>);
static_assert(std::convertible_to<xranges::iterator_t<const COV>,
    xranges::iterator_t<COV>>);
static_assert(!std::convertible_to<xranges::iterator_t<COV>,
              xranges::iterator_t<const COV>>);
...
static_assert(
    std::common_reference_with<lwg4074::Reference, lwg4074::CommonReference>);
static_assert(std::forward_iterator<lwg4074::Iter>);
static_assert(xranges::forward_range<lwg4074::PatternWithProxyConstAccess>);
static_assert(
    xranges::forward_range<lwg4074::PatternWithProxyConstAccess const>);
} // namespace selftest
