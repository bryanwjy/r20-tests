...
//   constexpr subrange<I> ranges::find_last_if_not(I first, S last, Pred pred,
//   Proj proj = {});
// template<forward_range R, class Proj = identity,
//          indirect_unary_predicate<projected<iterator_t<R>, Proj>> Pred>
//   constexpr borrowed_subrange_t<R> ranges::find_last_if_not(R&& r, Pred pred,
//   Proj proj = {});
...
concept HasFindLastIfIt = requires(
    It it, Sent sent) { xranges::find_last_if_not(it, sent, Predicate{}); };
...
concept HasFindLastIfPred =
    requires(int* it, Pred pred) { xranges::find_last_if_not(it, it, pred); };
...
concept HasFindLastIfR =
    requires(R r) { xranges::find_last_if_not(r, Predicate{}); };
...
constexpr void test_iterator_classes() {
    ...
    { // check that an empty range works
...

        {
            ...
            auto ret = xranges::find_last_if_not(
        ...
        }
        {
            ...
            auto ret =
                xranges::find_last_if_not(make_range<it, sent>(a), [](auto&&) {
                    return false;
                }).begin();
            assert(ret == it(a.end()));
        }
    ...
    { // check that last is returned with no match
...

        {
            ...
            auto ret =
                xranges::find_last_if_not(it(a), sent(it(a + 3)), [](auto&&) {
                    return true;
                }).begin();
            assert(ret == it(a + 3));
        }
...
constexpr bool test() {
...

    {
        ...
        [[maybe_unused]] std::same_as<xranges::dangling> auto ret =
            xranges::find_last_if_not(
                std::array{1, 2}, [](int) { return false; });
    }
...

    {
        // check that the return type of `iter::operator*` doesn't change
        {
            ...
            auto ret = xranges::find_last_if_not(a, a + 1, [](auto&& e) {
        ...
        }
        {
            ...
            auto ret = xranges::find_last_if_not(a, [](auto&& e) {
                return e != NonConstComparable{};
            }).begin();
            assert(ret == a);
        }
