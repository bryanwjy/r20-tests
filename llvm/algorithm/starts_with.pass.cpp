...
// template<input_range R1, input_range R2, class Pred = ranges::equal_to, class
// Proj1 = identity,
//          class Proj2 = identity>
//   requires indirectly_comparable<iterator_t<R1>, iterator_t<R2>, Pred, Proj1,
//   Proj2> constexpr bool ranges::starts_with(R1&& r1, R2&& r2, Pred pred = {},
//                                      Proj1 proj1 = {}, Proj2 proj2 = {});
...
template <class Iter1, class Sent1 = Iter1, class Iter2 = int*,
    class Sent2 = Iter2>
concept HasStartsWithIt = requires(Iter1 first1, Sent1 last1, Iter2 first2,
    Sent2 last2) { xranges::starts_with(first1, last1, first2, last2); };
...
static_assert(
    !HasStartsWithIt<int*, SentinelForNotWeaklyEqualityComparableWith>);
...
static_assert(!HasStartsWithIt<int*, int*, int*,
              SentinelForNotWeaklyEqualityComparableWith>);

template <class Range1, class Range2 = UncheckedRange<int*>>
concept HasStartsWithR = requires(
    Range1 range1, Range2 range2) { xranges::starts_with(range1, range2); };
...
static_assert(
    HasStartsWithR<UncheckedRange<int*>, ForwardRangeNotIncrementable>);
static_assert(
    !HasStartsWithR<UncheckedRange<int*>, ForwardRangeNotSentinelSemiregular>);
static_assert(!HasStartsWithR<UncheckedRange<int*>,
  ...
  {  // simple tests
    {
      ...
      std::same_as<bool> decltype(auto) ret =
        xranges::starts_with(Iter1(a), Sent1(Iter1(a + 6)), Iter2(p), Sent2(Iter2(p + 2)));
      assert(ret);
    }
    {
      int a[]                               = {1, 2, 3, 4, 5, 6};
      int p[]                               = {1, 2};
      auto whole                            = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
      auto prefix                           = xranges::subrange(Iter2(p), Sent2(Iter2(p + 2)));
      std::same_as<bool> decltype(auto) ret = xranges::starts_with(whole, prefix);
      assert(ret);
    }
  }
  ...
  { // prefix doesn't match
    {
      ...
      int p[] = {4, 5, 6};
      std::same_as<bool> decltype(auto) ret =
          xranges::starts_with(Iter1(a), Sent1(Iter1(a + 6)), Iter2(p), Sent2(Iter2(p + 3)));
      assert(!ret);
    }
    {
      int a[]                               = {1, 2, 3, 4, 5, 6};
      int p[]                               = {4, 5, 6};
      auto whole                            = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
      auto prefix                           = xranges::subrange(Iter2(p), Sent2(Iter2(p + 3)));
      std::same_as<bool> decltype(auto) ret = xranges::starts_with(whole, prefix);
      assert(!ret);
    }
  }
  ...
  { // range and prefix are identical
    {
      ...
      int p[] = {1, 2, 3, 4, 5, 6};
      std::same_as<bool> decltype(auto) ret =
          xranges::starts_with(Iter1(a), Sent1(Iter1(a + 6)), Iter2(p), Sent2(Iter2(p + 6)));
      assert(ret);
    }
    {
      int a[]                               = {1, 2, 3, 4, 5, 6};
      int p[]                               = {1, 2, 3, 4, 5, 6};
      auto whole                            = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
      auto prefix                           = xranges::subrange(Iter2(p), Sent2(Iter2(p + 6)));
      std::same_as<bool> decltype(auto) ret = xranges::starts_with(whole, prefix);
      assert(ret);
    }
  }
  ...
  { // prefix is longer than range
    {
      ...
      std::same_as<bool> decltype(auto) ret =
          xranges::starts_with(Iter1(a), Sent1(Iter1(a + 6)), Iter2(p), Sent2(Iter2(p + 8)));
      assert(!ret);
    }
    {
      int a[]                               = {1, 2, 3, 4, 5, 6};
      int p[]                               = {1, 2, 3, 4, 5, 6, 7, 8};
      auto whole                            = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
      auto prefix                           = xranges::subrange(Iter2(p), Sent2(Iter2(p + 8)));
      std::same_as<bool> decltype(auto) ret = xranges::starts_with(whole, prefix);
      assert(!ret);
    }
  }
  ...
  { // prefix has zero length
    {
      ...
      std::array<int, 0> p = {};
      std::same_as<bool> decltype(auto) ret =
          xranges::starts_with(Iter1(a), Sent1(Iter1(a + 6)), Iter2(p.data()), Sent2(Iter2(p.data())));
      assert(ret);
    }
    {
      int a[]                               = {1, 2, 3, 4, 5, 6};
      std::array<int, 0> p = {};
      auto whole                            = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
      auto prefix                           = xranges::subrange(Iter2(p.data()), Sent2(Iter2(p.data())));
      std::same_as<bool> decltype(auto) ret = xranges::starts_with(whole, prefix);
      assert(ret);
    }
  }
  ...
  { // range has zero length
    {
      ...
      int p[] = {1, 2, 3, 4, 5, 6, 7, 8};
      std::same_as<bool> decltype(auto) ret =
          xranges::starts_with(Iter1(a.data()), Sent1(Iter1(a.data())), Iter2(p), Sent2(Iter2(p + 8)));
      assert(!ret);
    }
    {
      std::array<int, 0> a = {};
  ...
  { // check that the predicate is used
    {
      int a[] = {11, 8, 3, 4, 0, 6};
      int p[]                               = {1, 12};
      std::same_as<bool> decltype(auto) ret = xranges::starts_with(
    ...
    }
    {
      ...
      auto whole                            = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
      auto prefix                           = xranges::subrange(Iter2(p), Sent2(Iter2(p + 2)));
      std::same_as<bool> decltype(auto) ret = xranges::starts_with(whole, prefix, [](int l, int r) { return l > r; });
      assert(!ret);
    }
  }
...
constexpr bool test() {
  ...
  { // check that std::invoke is used
    ...
    };
    {
      S a[]    = {{1}, {2}, {3}, {4}};
      S p[]    = {{1}, {2}};
      auto ret = xranges::starts_with(a, a + 4, p, p + 2, &S::compare, &S::identity, &S::identity);
      assert(ret);
    }
    {
      S a[]    = {{1}, {2}, {3}, {4}};
      S p[]    = {{1}, {2}};
      auto ret = xranges::starts_with(a, p, &S::compare, &S::identity, &S::identity);
      assert(ret);
    }
  }
