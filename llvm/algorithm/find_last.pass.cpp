...
// MSVC warning C4242: 'argument': conversion from 'const _Ty' to 'ElementT',
// possible loss of data MSVC warning C4244: 'argument': conversion from 'const
// _Ty' to 'ElementT', possible loss of data
// ADDITIONAL_COMPILE_FLAGS(cl-style-warnings): /wd4242 /wd4244

// template<forward_iterator I, sentinel_for<I> S, class T, class Proj =
// identity>
//   requires indirect_binary_predicate<ranges::equal_to, projected<I, Proj>,
//   const T*> constexpr subrange<I> ranges::find_last(I first, S last, const T&
//   value, Proj proj = {});
// template<forward_range R, class T, class Proj = identity>
//   requires indirect_binary_predicate<ranges::equal_to,
//   projected<iterator_t<R>, Proj>, const T*> constexpr borrowed_subrange_t<R>
...
concept HasFindLastIt =
    requires(It it, Sent sent) { xranges::find_last(it, sent, *it); };
...
concept HasFindLastR = requires(Range r) { xranges::find_last(r, ValT{}); };
static_assert(HasFindLastR<std::array<int, 1>, int>);
static_assert(!HasFindLastR<int, int>);
static_assert(
    !HasFindLastR<std::array<NotEqualityComparable, 1>, NotEqualityComparable>);
...
static_assert(
    !HasFindLastR<ForwardRangeNotSentinelEqualityComparableWith, int>);

template <class It, class Sent = It>
constexpr void test_iterators() {
    using ValueT = std::iter_value_t<It>;
    auto make_range = [](auto& a) {
        return xranges::subrange(It(std::to_address(xranges::begin(a))),
            Sent(It(std::to_address(xranges::end(a)))));
    };
    { // simple test
        {
            ValueT a[] = {1, 2, 3, 4};
            ...
            std::same_as<xranges::subrange<It>> auto ret =
                xranges::find_last(It(a), Sent(It(a + 4)), 2);
        ...
        }
        {
            ...
            std::same_as<xranges::subrange<It>> auto ret =
                xranges::find_last(make_range(a), 2);
            assert(base(ret.begin()) == a + 1);
            assert(*ret.begin() == 2);
        }
    }
    ...
    { // check that an empty range works
        {
            ...
            auto ret =
                xranges::find_last(It(a.data()), Sent(It(a.data())), 1).begin();
            assert(ret == It(a.data()));
        }
        {
            std::array<ValueT, 0> a = {};

            auto ret = xranges::find_last(make_range(a), 1).begin();
            assert(ret == It(a.data()));
        }
    }
    ...
    { // check that last is returned with no match
        {
            ...
            auto ret = xranges::find_last(It(a), Sent(It(a + 3)), 0).begin();
            assert(ret == It(a + 3));
        }
        {
            ValueT a[] = {1, 1, 1};

            auto ret = xranges::find_last(make_range(a), 0).begin();
            assert(ret == It(a + 3));
        }
    }
}
...
constexpr void test_iterator_classes() {
    { // check that the last element is returned
        ...
        auto ret =
            xranges::find_last(it(std::begin(a)), it(std::end(a)), 0, &S::comp)
                .begin();
        assert(ret == it(a + 2));
        assert((*ret).comp == 0);
        assert((*ret).other == 1);
...

    {
        ...
        auto ret = xranges::find_last(
            it(std::begin(a)), it(std::end(a)), 2, [&](int i) {
                ++projection_count;
                return i;
            }).begin();
        assert(ret == it(a + 1));
...
public:
    ...
    bool operator==(TriviallyComparable const&) const = default;
};

constexpr bool test() {
    types::for_each(
        types::type_list<char, int, TriviallyComparable<char>>{}, []<class T> {
...

    {
        ...
        auto view = vec | xviews::join;
        assert(xranges::find_last(view.begin(), view.end(), 4).begin() ==
            std::next(view.begin(), 3));
        assert(
            xranges::find_last(view, 4).begin() == std::next(view.begin(), 3));
    }

    {
        ...
        std::same_as<xranges::subrange<int*>> auto ret =
            xranges::find_last(xviews::all(a), 1);
        assert(ret.begin() == a);
        assert(*ret.begin() == 1);
    }

    {
        ...
        std::same_as<xranges::dangling> auto ret =
            xranges::find_last(std::vector<int>(), 0);
        (void)ret;
    }
