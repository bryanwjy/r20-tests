...
//   Proj2> constexpr bool ranges::ends_with(R1&& r1, R2&& r2, Pred pred = {},
//                                      Proj1 proj1 = {}, Proj2 proj2 = {});
...
template <class Iter1, class Sent1 = Iter1, class Iter2 = int*,
    class Sent2 = Iter2>
concept HasEndsWithIt = requires(Iter1 first1, Sent1 last1, Iter2 first2,
    Sent2 last2) { xranges::ends_with(first1, last1, first2, last2); };
...
constexpr void test_iterators() {
    { // simple tests
        ...
        int p[] = {4, 5, 6};
        {
            ...
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 3)));
            [[maybe_unused]] std::same_as<bool> decltype(auto) ret =
                xranges::ends_with(
                    whole.begin(), whole.end(), suffix.begin(), suffix.end());
            assert(ret);
        }
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 3)));
            [[maybe_unused]] std::same_as<bool> decltype(auto) ret =
                xranges::ends_with(whole, suffix);
            assert(ret);
        }
    }
    ...
    { // suffix doesn't match
        ...
        }
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 3)));
            bool ret = xranges::ends_with(whole, suffix);
            assert(!ret);
        }
    }
    ...
    { // range consists of just one element
        ...
        int p[] = {1};
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 1)));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 1)));
            bool ret = xranges::ends_with(
                whole.begin(), whole.end(), suffix.begin(), suffix.end());
            assert(ret);
        }
    ...
    { // suffix consists of just one element
        ...
        int p[] = {3};
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 5)));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 1)));
            bool ret = xranges::ends_with(
                whole.begin(), whole.end(), suffix.begin(), suffix.end());
            assert(ret);
    ...
    { // range and suffix are identical
        ...
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 6)));
            bool ret = xranges::ends_with(
                whole.begin(), whole.end(), suffix.begin(), suffix.end());
            assert(ret);
        }
    ...
    { // suffix is longer than range
        ...
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 8)));
            bool ret = xranges::ends_with(
                whole.begin(), whole.end(), suffix.begin(), suffix.end());
            assert(!ret);
        }
    ...
    { // range has zero length
        ...
        {
            auto whole =
                xranges::subrange(Iter1(a.data()), Sent1(Iter1(a.data())));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 8)));
            bool ret = xranges::ends_with(
                whole.begin(), whole.end(), suffix.begin(), suffix.end());
            assert(!ret);
    ...
    { // subarray
        ...
        int p[] = {3, 5};
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 13)));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 2)));
            bool ret = xranges::ends_with(
                whole.begin(), whole.end(), suffix.begin(), suffix.end());
            assert(!ret);
        }
    ...
    { // repeated suffix
        ...
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 4)));
            bool ret = xranges::ends_with(
                whole.begin(), whole.end(), suffix.begin(), suffix.end());
            assert(!ret);
        }
    ...
    { // check that the predicate is used
        ...
        auto pred = [](int l, int r) { return l * -1 == r; };
        {
            ...
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 2)));
            bool ret = xranges::ends_with(
                whole.begin(), whole.end(), suffix.begin(), suffix.end(), pred);
            assert(ret);
        }
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 5)));
            auto suffix = xranges::subrange(Iter2(p), Sent2(Iter2(p + 2)));
            bool ret = xranges::ends_with(whole, suffix, pred);
            assert(ret);
        }
    }
    ...
    { // check that the projections are used
        ...
        int p[] = {2, 1, 2};
        {
            ...
            bool ret = xranges::ends_with(
                whole.begin(), whole.end(), suffix.begin(), suffix.end(), {},
                [](int i) { return i - 3; }, [](int i) { return i * -1; });
            assert(ret);
        }
        {
            auto whole = xranges::subrange(Iter1(a), Sent1(Iter1(a + 6)));
...
constexpr bool test() {
    // This is to test (forward_iterator<_Iter1> || sized_sentinel_for<_Sent1,
    // _Iter1>) condition.
    types::for_each(
        types::cpp20_input_iterator_list<int*>{}, []<class Iter2>() {
            types::for_each(
                types::cpp20_input_iterator_list<int*>{}, []<class Iter1>() {
                    ...
                    if constexpr (std::forward_iterator<Iter2>)
                        test_iterators<Iter1, sized_sentinel<Iter1>, Iter2,
                            Iter2>();
                    if constexpr (std::forward_iterator<Iter1>)
                        test_iterators<Iter1, Iter1, Iter2,
