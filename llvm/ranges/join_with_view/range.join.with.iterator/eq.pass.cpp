...
constexpr bool test() {
    { // `V` and `Pattern` are not empty. Test return types too.
        ...
        // `it1.inner_it_` and `it2.inner_it_` are equal, `it1.outer_it_` and
        // `it2.outer_it_` are equal too. `it1.inner_it_index()` and
        ...
        // `cit1.inner_it_` and `cit2.inner_it_` are equal, `cit1.outer_it_` and
        // `cit2.outer_it_` are equal too. `cit1.inner_it_index()` and
        // `cit2.inner_it_index()` are equal to 1.
        ++cit1;
        assert(*cit1 == 7);
        assert(testEquality(it1, cit1, true));
        xranges::advance(cit2, -2);
        ...
        // `it1.inner_it_` and `it2.inner_it_` are equal, `it1.outer_it_` and
        // `it2.outer_it_` are equal too. `it1.inner_it_index()` and
        // `it2.inner_it_index()` are equal to 0.
        --it1;
        assert(*it1 == 0);
        --it2;
        assert(*it2 == 0);
        ...
        // `cit1.inner_it_` and `cit2.inner_it_` are equal, `cit1.outer_it_` and
        // `cit2.outer_it_` are equal too. `cit1.inner_it_index()` and
        // `cit2.inner_it_index()` are equal to 0.
        --cit1;
        assert(*cit1 == 0);
        assert(testEquality(it1, cit1, true));
        --cit2;
    ...
    { // `InnerIter` models input iterator and equality comparable. `Pattern` is
      // empty.
    ...
    { // `Pattern` is not empty. Some elements of `V` are.
        ...
        using Pattern = BasicVectorView<int, ViewProperties{.common = false},
            forward_iterator>;
