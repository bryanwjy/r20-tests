...
// >=, <=, !=. The spec indicates that zip iterator's >= is negating zip
// iterator's < instead of calling underlying iterator's >=. Declare all the
// operations >, >=, <= etc to make it satisfy random_access_iterator concept,
// but not define them. If the zip iterator's >,>=, <=, etc isn't implemented in
// the way defined by the standard but instead calling underlying iterator's
// >,>=,<=, we will get a linker error for the runtime tests and non-constant
// expression for the compile time tests.
struct LessThanIterator {
    ...
    constexpr LessThanIterator operator++(int) {
    ...
    constexpr friend LessThanIterator operator+(
        LessThanIterator x, difference_type n) {
        x += n;
        return x;
    }
    constexpr friend LessThanIterator operator+(
        difference_type n, LessThanIterator x) {
        x += n;
        return x;
    }
    constexpr friend LessThanIterator operator-(
    ...
    constexpr friend difference_type operator-(
        LessThanIterator x, LessThanIterator y) {
        return x.it_ - y.it_;
    }

    constexpr friend bool operator==(
        LessThanIterator const&, LessThanIterator const&) = default;
    friend bool operator!=(LessThanIterator const&, LessThanIterator const&);

    constexpr friend bool operator<(
        LessThanIterator const& x, LessThanIterator const& y) {
        return x.it_ < y.it_;
    }
    friend bool operator<=(LessThanIterator const&, LessThanIterator const&);
    friend bool operator>(LessThanIterator const&, LessThanIterator const&);
...
static_assert(xranges::random_access_range<SmallerThanRange>);
...
constexpr bool test() {
    {
        ...
        int b[] = {5, 6, 7, 8, 9};
        auto r =
            xviews::zip(SubRange(It(a), It(a + 4)), SubRange(It(b), It(b + 5)));
        auto iter1 = r.begin();
        auto iter2 = iter1 + 1;
    ...
    {
        // in this case sentinel is computed by getting each of the underlying
        // sentinel, so only one underlying iterator is comparing equal
        int buffer1[1] = {1};
        int buffer2[2] = {1, 2};
        xranges::zip_view v{
            ForwardCommonView(buffer1), ForwardCommonView(buffer2)};
        using View = decltype(v);
        static_assert(xranges::common_range<View>);
...

    {
        ...
        int a[] = {1, 2, 3, 4};
        int b[] = {5, 6, 7, 8, 9};
        auto r = xviews::zip(SmallerThanRange(a), SmallerThanRange(b));
        auto iter1 = r.begin();
        auto iter2 = iter1 + 1;
...

    {
        ...
        using IterNoEqualView = BasicView<cpp20_input_iterator<int*>,
            sentinel_wrapper<cpp20_input_iterator<int*>>>;
        int buffer[] = {1};
        xranges::zip_view r(IterNoEqualView{buffer});
        auto it = r.begin();
