...
// x - n;
// x - y;
// All the arithmetic operators have the constraint `requires
// all-random-access<Const, Views...>;`, except `operator-(x, y)` which instead
// has the constraint
...
constexpr bool test() {
    ...
    static_assert(xranges::random_access_range<decltype(a)>);
    std::array b{4.1, 3.2, 4.3, 0.1, 0.2};
    static_assert(xranges::contiguous_range<decltype(b)>);
    {
...

    {
        ...
        // sentinels, so the distance between begin and end for each of the
        // underlying iterators can be different
        xranges::zip_view v{
...

    {
        ...
        xranges::zip_view v(a, b, ForwardSizedView{buffer1});
...

    {
        ...
        xranges::zip_view v(a, b, InputCommonView{buffer1});
        using Iter = decltype(v.begin());
        static_assert(!std::invocable<std::minus<>, Iter, Iter>);
    }
