...
concept implicitly_constructible_from =
    requires(Args&&... args) { conversion_test<T>({std::move(args)...}); };

// test constructor is explicit
static_assert(
    std::constructible_from<xranges::zip_view<SimpleCommon>, SimpleCommon>);
static_assert(!implicitly_constructible_from<xranges::zip_view<SimpleCommon>,
    SimpleCommon>);

static_assert(
    std::constructible_from<xranges::zip_view<SimpleCommon, SimpleCommon>,
        SimpleCommon, SimpleCommon>);
static_assert(!implicitly_constructible_from<
    xranges::zip_view<SimpleCommon, SimpleCommon>, SimpleCommon, SimpleCommon>);
...
struct MoveAwareView : xranges::view_base {
    ...
    constexpr int const* begin() const { return &moves; }
    constexpr int const* end() const { return &moves + 1; }
};

template <class View1, class View2>
constexpr void constructorTest(auto&& buffer1, auto&& buffer2) {
    xranges::zip_view v{View1{buffer1}, View2{buffer2}};
    auto [i, j] = *v.begin();
    assert(i == buffer1[0]);
    assert(j == buffer2[0]);
};
...
constexpr bool test() {
...

    {
        ...
        MoveAwareView mv;
        xranges::zip_view v{std::move(mv), MoveAwareView{}};
        auto [numMoves1, numMoves2] = *v.begin();
        assert(numMoves1 == 2); // one move from the local variable to
                                // parameter, one move from parameter to member
        assert(numMoves2 == 1);
    }
    ...
    // contiguous
    {
        constructorTest<ContiguousCommonView, ContiguousCommonView>(
            buffer, buffer2);
    }
