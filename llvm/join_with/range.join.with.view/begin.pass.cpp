...
using MaybeSimpleForwardView = BasicView<std::vector<std::string>,
    ViewProperties{.simple = Simple}, forward_iterator>;

template <bool Simple>
using MaybeSimpleForwardRvalueView = BasicView<RvalueVector<std::string>,
    ViewProperties{.simple = Simple}, forward_iterator>;

template <bool Simple>
using MaybeSimplePattern =
    BasicView<std::string, ViewProperties{.simple = Simple}, forward_iterator>;

template <class V, class Pattern>
concept JoinWithViewHasConstBegin =
    requires(xranges::join_with_view<V, Pattern> const jwv) {
...
constexpr void test_begin() {
    ...
    { // `V` does not model simple-view
        ...
        using Pattern = MaybeSimplePattern<true>;
        xranges::join_with_view<V, Pattern> jwv(V(Vec{}), Pattern(Str{}));
        auto it = jwv.begin();
        assert(it == jwv.end());
    }
    ...
    { // `V` models simple-view
        ...
        xranges::join_with_view<V, Pattern> jwv(V(Vec{"07"}), Pattern(Str{}));
        auto it = jwv.begin();
        assert(*it++ == '0');
        assert(*it == '7');
    }
    ...
    { // `V` does not model forward range
        ...
        using V = BasicView<Vec, ViewProperties{.common = false},
...
constexpr void test_const_begin() {
    ...
    // `range_reference_t<const V>` models input range
    static_assert(
        !JoinWithViewHasConstBegin<BasicView<RvalueVector<std::vector<int>>,
    ...
    // are the same
    using JWV = xranges::join_with_view<MaybeSimpleForwardView<true>,
        MaybeSimplePattern<true>>;
    static_assert(std::same_as<xranges::iterator_t<JWV&>,
        xranges::iterator_t<const JWV&>>);
}
