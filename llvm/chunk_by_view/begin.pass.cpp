...
static_assert(HasBegin<xranges::chunk_by_view<Range, TrackingPred>>);
static_assert(!HasBegin<xranges::chunk_by_view<Range, TrackingPred> const>);
...
constexpr bool test() {
    ...
    // Check the return type of `begin()`
    {
        ...
        using ChunkByIterator = xranges::iterator_t<decltype(view)>;
    ...
    // (we should be passing it to ranges::adjacent_find using std::ref)
    {
        ...
        Range range(buff, buff + 2);
        xranges::chunk_by_view view(range, TrackingPred(&moved, &copied));
    ...
    // Test caching
    {
        ...
        for (int k = 0; k != 3; ++k) {
            auto it = view.begin();
            assert(base((*it).end()) == buff + 4);
            assert(called == 4); // 4, because the cached iterator is 'buff + 4'
