...
struct UnwrapAdaptor : Adaptor {
    ...
    decltype(auto) get_container() {
        return (UnwrapAdaptor::c); // Put into parentheses to make sure the
                                   // function returns a reference.
    }

    // TODO: make this work pre-C++20.
    decltype(auto) get_comparator()
    requires requires { UnwrapAdaptor::c; }
    {
        return (UnwrapAdaptor::comp); // Put into parentheses to make sure the
                                      // function returns a reference.
    }
};
