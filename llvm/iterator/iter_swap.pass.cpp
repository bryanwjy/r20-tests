...
//   friend constexpr void iter_swap(const common_iterator& x, const
//   common_iterator<I2, S2>& y)
//     noexcept(noexcept(ranges::iter_swap(declval<const I&>(), declval<const
//     I2&>())));
...
struct IterSwappingIt {
    ...
    IterSwappingIt(
        IterSwappingIt const&); // copyable, but this test shouldn't make copies
    IterSwappingIt(IterSwappingIt&&) = default;
    IterSwappingIt& operator=(IterSwappingIt const&);
    int& operator*() const;
    constexpr IterSwappingIt& operator++() { return *this; }
    IterSwappingIt operator++(int);

    template <int L>
    friend constexpr int iter_swap(
        IterSwappingIt<K> const& lhs, IterSwappingIt<L> const& rhs) {
        *lhs.swaps_ += 10;
        *rhs.swaps_ += 1;
...
constexpr bool test() {
    ...
    }
    {
        ...
        assert(jswaps == 101);
        iter_swap(
            CommonIt(It(&iswaps)), CommonIt(It(&jswaps))); // rvalue iterators
        assert(iswaps == 120);
        assert(jswaps == 102);
    ...
    }
    {
        ...
        assert(jswaps == 101);
        iter_swap(
            CommonIt(It(&iswaps)), CommonJt(Jt(&jswaps))); // rvalue iterators
        assert(iswaps == 120);
        assert(jswaps == 102);
        xranges::iter_swap(it, jt);
        assert(iswaps == 130);
