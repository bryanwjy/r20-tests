...
using SplitView =
    rxx::ranges::lazy_split_view<std::string_view, std::string_view>;
...
static_assert(rxx::ranges::details::tiny_range<TinyView>);
static_assert(std::ranges::view<TinyView>);

using SplitView = rxx::ranges::lazy_split_view<InputView, TinyView>;
...
static_assert(
    sizeof(rxx::ranges::lazy_split_view<EmptyView1, EmptyView2>) == 1);
...
static_assert(rxx::ranges::details::tiny_range<EmptyTinyView>);
static_assert(std::ranges::view<EmptyTinyView>);

static_assert(
    sizeof(rxx::ranges::lazy_split_view<EmptyInputView, EmptyTinyView>) ==
    sizeof(rxx::ranges::details::non_propagating_cache<
        std::ranges::iterator_t<EmptyInputView>>));
