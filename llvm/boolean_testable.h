...
struct BooleanTestable {
    ...
    friend constexpr BooleanTestable operator==(
        BooleanTestable const& lhs, BooleanTestable const& rhs) {
    ...
    // this class should behave like a bool, so the constructor shouldn't be
    // explicit
    constexpr BooleanTestable(bool value) : value_{value} {}
    constexpr BooleanTestable(BooleanTestable const&) = delete;
...
struct StrictComparable {
    ...
    // this shouldn't be explicit to make it easier to initialize inside arrays
    // (which it almost always is)
    constexpr StrictComparable(T value) : value_{value} {}

    friend constexpr BooleanTestable const& operator==(
        StrictComparable const& a, StrictComparable const& b) {
...
auto StrictUnaryPredicate =
    []<class T>(StrictComparable<T> const& x) -> BooleanTestable const& {
    return x.value_ < 0 ? yes : no;
};

auto StrictBinaryPredicate =
    []<class T>(StrictComparable<T> const& x,
        StrictComparable<T> const& y) -> BooleanTestable const& {
...
struct StrictBooleanIterator {
    ...
    friend constexpr StrictBooleanIterator operator+(
        StrictBooleanIterator x, difference_type n) {
        x += n;
        return x;
    }
    friend constexpr StrictBooleanIterator operator+(
        difference_type n, StrictBooleanIterator x) {
        x += n;
        return x;
    }
    friend constexpr StrictBooleanIterator operator-(
    ...
    friend constexpr difference_type operator-(
        StrictBooleanIterator x, StrictBooleanIterator y) {
        return x.iter_ - y.iter_;
    }
    constexpr BooleanTestable const& operator==(
        StrictBooleanIterator const& other) const {
...
static_assert(std::sentinel_for<StrictBooleanIterator<int*>,
    StrictBooleanIterator<int*>>);
