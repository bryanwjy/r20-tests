...
public:
    constexpr invocable_with_telemetry(F f, invocable_telemetry& telemetry)
        : f_(f)
    ...
    requires std::move_constructible<F>
        ...
        , telemetry_((assert(other.telemetry_ != nullptr),
              std::exchange(other.telemetry_, nullptr))) {
        ++telemetry_->moves;
    }
    ...
    requires std::copy_constructible<F>
        : f_(other.f_)
        , telemetry_((assert(other.telemetry_ != nullptr), other.telemetry_)) {
        ++telemetry_->copies;
    }

    constexpr invocable_with_telemetry& operator=(
        invocable_with_telemetry&& other)
    requires std::movable<F>
    {
        // Not using move-and-swap idiom to ensure that copies and moves remain
        // accurate.
        assert(&other != this);
    ...
    constexpr invocable_with_telemetry& operator=(
        invocable_with_telemetry const& other)
    requires std::copyable<F>
    {
        // Not using copy-and-swap idiom to ensure that copies and moves remain
        // accurate.
        assert(&other != this);
        assert(other.telemetry_ != nullptr);
...
template <class F>
invocable_with_telemetry(F f, int& invocations, int& moves, int& copies)
    -> invocable_with_telemetry<F>;
