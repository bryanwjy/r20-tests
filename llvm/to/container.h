...
// different constructors -- e.g., if 3 constructors are available, the
// `from_range_t` constructor is chosen in favor of the constructor taking two
// iterators, etc.
template <class ElementType, CtrChoice Rank,
    InserterChoice Inserter = InserterChoice::Insert, bool CanReserve = false>
struct Container {
    ...
    constexpr Container(
        std::from_range_t, xranges::input_range auto&& in, int arg1, char arg2)
    requires (Rank >= CtrChoice::FromRangeT)
    ...
    friend constexpr bool operator==(
        Container const&, Container const&) = default;
};
