...
struct Y2 {
    Y2() = default;
    Y2(int const&) = delete;
    Y2& operator=(int const&) { return *this; }
};
...
template <class T>
int AssignableFrom<T>::type_constructed = 0;
template <class T>
int AssignableFrom<T>::type_assigned = 0;
template <class T>
int AssignableFrom<T>::int_constructed = 0;
template <class T>
int AssignableFrom<T>::int_assigned = 0;
...
void test_ambiguous_assign() {
    using OptInt = __RXX optional<int>;
    {
        ...
        }
        {
            ...
            static_assert(!std::is_assignable<Opt&, OptInt const&&>::value, "");
            static_assert(!std::is_assignable<Opt&, OptInt const&>::value, "");
...
constexpr bool test() {
    ...
    class StateTracker {
    public:
        constexpr StateTracker(state_t& s) : state_(&s) {
    ...
    };
    {
        ...
        auto opt1 = __RXX optional<StateTracker>(state[0]);
        assert(state[0] == state_t::constructed);

        auto opt2 = __RXX optional<StateTracker>(state[1]);
