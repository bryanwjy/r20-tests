...
struct ThrowAssign {
    ...
    ThrowAssign() = default;
    ThrowAssign(int) { RXX_THROW(42); }
    ThrowAssign& operator=(int) { RXX_THROW(42); }
...
void assert_assignable() {
    ...
    static_assert(!std::is_assignable<optional<T> const&, Arg>::value, "");
}
...
struct FromOptionalType {
    using Opt = __RXX optional<FromOptionalType>;
    ...
    template <class Dummy = void>
    constexpr FromOptionalType& operator=(Opt&) {
        Dummy::BARK;
        return *this;
    }
};
...
void test_sfinae() {
    ...
    assert_assignable<FromOptionalType, __RXX optional<FromOptionalType>&,
        false>();
}
...
constexpr void test_with_ref() {
    ...
    // test two objects, make sure that the optional only changes what it holds
    // a reference to
    {
        T t2{_Val};
        optional<T&> opt{t};
    ...
    // test that reassigning the reference for an optional<T&> doesn't affect
    // the objet it's holding a reference to
    {
        int i = -1;
        int j = 2;
        optional<int&> opt{i};
