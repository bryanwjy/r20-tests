...
int main(int, char**) {
    {
        static_assert(
            std::is_nothrow_move_assignable<optional<int>>::value, "");
        optional<int> opt;
    ...
    { static_assert(std::is_nothrow_move_assignable<optional<Y>>::value, ""); }
    {
        ...
        static_assert(
            !std::is_nothrow_move_assignable<optional<ThrowsMove>>::value, "");
        struct ThrowsMoveAssign {
            ...
            ThrowsMoveAssign& operator=(ThrowsMoveAssign&&) noexcept(false) {
                return *this;
            }
        };
        static_assert(
            !std::is_nothrow_move_assignable<optional<ThrowsMoveAssign>>::value,
            "");
        struct NoThrowMove {
        ...
        static_assert(
            std::is_nothrow_move_assignable<optional<NoThrowMove>>::value, "");
    }
    return 0;
}
