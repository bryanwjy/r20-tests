...
void test(InitArgs&&... args) {
    optional<T> const orig(std::forward<InitArgs>(args)...);
    optional<T> rhs(orig);
...
#  if 0 // FIXME: optional<T&&> is not allowed.
  ...
  assert(T::destroyed == 1);
  {
    ...
    static_assert(!std::is_copy_constructible_v<__RXX optional<T const&&>>);
  }
#  endif
#endif
}
...
int main(int, char**) {
    ...
    {
        optional<int const> o(42);
        optional<int const> o2(std::move(o));
        assert(*o2 == 42);
    }
    {
        using T = TestTypes::TestType;
        ...
        optional<T> rhs;
        assert(T::alive == 0);
        optional<T> const lhs(std::move(rhs));
    ...
    { test_throwing_ctor(); }
    {
        ...
        static_assert(
            !std::is_nothrow_move_constructible<optional<ThrowsMove>>::value,
            "");
        struct NoThrowMove {
        ...
        static_assert(
            std::is_nothrow_move_constructible<optional<NoThrowMove>>::value,
            "");
    }
    { test_reference_extension(); }
    {
        constexpr __RXX optional<int> o1{4};
        constexpr __RXX optional<int> o2 = std::move(o1);
        static_assert(*o2 == 4, "");
    }
