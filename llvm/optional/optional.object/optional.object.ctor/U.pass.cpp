...
constexpr bool implicit_conversion(optional<To>&& opt, From const& v) {
    using O = optional<To>;
...
constexpr bool explicit_conversion(Input&& in, Expect const& v) {
    using O = optional<To>;
...
void test_implicit() {
    ...
    }
    {
        ...
        static_assert(!test_convertible<O, std::in_place_t&>(), "");
        static_assert(!test_convertible<O, std::in_place_t const&>(), "");
        static_assert(!test_convertible<O, std::in_place_t&&>(), "");
        static_assert(!test_convertible<O, std::in_place_t const&&>(), "");
    }
