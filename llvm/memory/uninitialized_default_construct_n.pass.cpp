...
// ForwardIterator ranges::uninitialized_default_construct_n(ForwardIterator
// first,
...
static_assert(
    std::is_class_v<decltype(xranges::uninitialized_default_construct_n)>);

struct NotDefaultCtrable {
    NotDefaultCtrable() = delete;
};
static_assert(
    !std::is_invocable_v<decltype(xranges::uninitialized_default_construct_n),
...
int main(int, char**) {
    // An empty range -- no default constructors should be invoked.
    {
        ...
        xranges::uninitialized_default_construct_n(buf.begin(), 0);
    ...
    // A range containing several objects.
    {
        ...
        xranges::uninitialized_default_construct_n(buf.begin(), N);
