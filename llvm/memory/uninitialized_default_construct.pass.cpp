...
// ForwardIterator ranges::uninitialized_default_construct(ForwardIterator
// first, Sentinel last);
//
...
// ranges::uninitialized_default_construct(ForwardRange&& range);
...
static_assert(
    std::is_class_v<decltype(xranges::uninitialized_default_construct)>);

struct NotDefaultCtrable {
    NotDefaultCtrable() = delete;
};
static_assert(
    !std::is_invocable_v<decltype(xranges::uninitialized_default_construct),
...
int main(int, char**) {
    // An empty range -- no default constructors should be invoked.
    {
        ...
        xranges::uninitialized_default_construct(buf.begin(), buf.begin());
        assert(Counted::current_objects == 0);
        assert(Counted::total_objects == 0);

        xranges::uninitialized_default_construct(
            xranges::empty_view<Counted>());
        assert(Counted::current_objects == 0);
        assert(Counted::total_objects == 0);
        ...
        xranges::uninitialized_default_construct(range.begin(), range.end());
        assert(Counted::current_objects == 0);
        assert(Counted::total_objects == 0);

        xranges::uninitialized_default_construct(range);
        assert(Counted::current_objects == 0);
        assert(Counted::total_objects == 0);
    }
    ...
    // A range containing several objects, (iter, sentinel) overload.
    {
        ...
        xranges::uninitialized_default_construct(buf.begin(), buf.end());
    ...
    // Using `counted_iterator`.
    {
        ...
        xranges::destroy(buf.begin(), buf.begin() + N);
        Counted::reset();
    }
    ...
    // Using `reverse_view`.
    {
        ...
        xranges::uninitialized_default_construct(xranges::reverse_view(range));
        assert(Counted::current_objects == N);
        assert(Counted::total_objects == N);
    ...
    // objects should stay valid. (range) overload.
    {
        ...
        try {
            xranges::uninitialized_default_construct(buf);
        } catch (...) {}
        assert(Counted::current_objects == 0);
        assert(Counted::total_objects == 3);
        xranges::destroy(buf.begin(), buf.begin() + Counted::total_objects);
