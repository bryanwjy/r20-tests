...
static_assert(
    !std::is_invocable_v<decltype(xranges::destroy_at), NotNothrowDtrable*>);
...
struct VirtualCountedDerived : VirtualCountedBase {
    constexpr VirtualCountedDerived(int& count_ref)
        : VirtualCountedBase(count_ref) {}

    // Without a definition, GCC gives an error when the destructor is invoked
    // in a constexpr context (see
...
constexpr bool test() {
    // Destroying a "trivial" object.
    {
        ...
        xranges::destroy_at(buffer);
        assert(counter == 1);
        xranges::destroy_at(buffer + 1);
        assert(counter == 0);
...
constexpr bool test_arrays() {
    ...
    // Pointer to a two-dimensional array.
    {
        ...
        for (int i = 0; i != 3; ++i) {
            for (int j = 0; j != 2; ++j) {
                Traits::construct(
                    alloc, std::addressof(array_ref[i][j]), counter);
            }
        }
        assert(counter == 3 * 2);
