...
// !weakly_incrementable<T>
static_assert(
    !HasIotaRange<UncheckedRange<int*>, WeaklyIncrementableNotMovable>);

// !ranges::output_range<const _Tp&>
static_assert(
    !HasIotaRange<UncheckedRange<int*>, OutputIteratorNotIndirectlyWritable>);
...
struct DangerousCopyAssign {
    ...
    // Needed in postfix
    constexpr DangerousCopyAssign(DangerousCopyAssign const& other) {
        this->val = other.val;
    }
    ...
    implementation. If for some reason it does invoke it, there will be a
    compiler error.
    */
    ...
    constexpr bool operator==(DangerousCopyAssign const& rhs) {
...
constexpr void test_result(std::array<int, N> input, int starting_value,
    std::array<int, N> const expected) {
    { // (iterator, sentinel) overload
        auto in_begin = Iter(input.data());
        auto in_end = Sent(Iter(input.data() + input.size()));
        std::same_as<xranges::out_value_result<Iter, int>> decltype(auto)
            result = xranges::iota(
                std::move(in_begin), std::move(in_end), starting_value);
        assert(result.out == in_end);
        assert(result.value == starting_value + static_cast<int>(N));
        assert(xranges::equal(input, expected));
    }

    { // (range) overload
        // in the range overload adds the additional constraint that it must be
        // an output range so skip this for the input iterators we test
        auto in_begin = Iter(input.data());
        auto in_end = Sent(Iter(input.data() + input.size()));
        auto range = xranges::subrange(std::move(in_begin), std::move(in_end));

        std::same_as<xranges::out_value_result<Iter, int>> decltype(auto)
            result = xranges::iota(range, starting_value);
        assert(result.out == in_end);
        assert(result.value == starting_value + static_cast<int>(N));
        assert(xranges::equal(input, expected));
    }
...
constexpr void test_user_defined_type() {
    ...
    struct UserDefinedType {
        ...
        constexpr UserDefinedType(UserDefinedType const& other) {
            this->val = other.val;
        }
    ...
    // Check
    assert(
        xranges::equal(a, expected, xranges::equal_to{}, proj_val, proj_val));
}
...
constexpr void test_dangerous_copy_assign() {
    ...
    assert(
        xranges::equal(aa, expected, xranges::equal_to{}, proj_val, proj_val));
}
