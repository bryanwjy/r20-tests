...
constexpr void test_emplace_sfinae() {
    using V = __RXX variant<int, TestTypes::NoCtors, InitList, InitListArg,
        long, long>;
    ...
    static_assert(
        !emplace_exists<V, InitListArg, IL, int, int>(), "too many args");
}
...
constexpr void test_basic() {
    using V = __RXX variant<int, InitList, InitListArg, NoCtor>;
    V v;
    auto& ref1 = v.emplace<InitList>({1, 2, 3});
    static_assert(std::is_same_v<InitList&, decltype(ref1)>, "");
    assert(__RXX get<InitList>(v).size == 3);
    assert(&ref1 == &__RXX get<InitList>(v));
    auto& ref2 = v.emplace<InitListArg>({1, 2, 3, 4}, 42);
    static_assert(std::is_same_v<InitListArg&, decltype(ref2)>, "");
    assert(__RXX get<InitListArg>(v).size == 4);
    assert(__RXX get<InitListArg>(v).value == 42);
    assert(&ref2 == &__RXX get<InitListArg>(v));
    auto& ref3 = v.emplace<InitList>({1});
    static_assert(std::is_same_v<InitList&, decltype(ref3)>, "");
    assert(__RXX get<InitList>(v).size == 1);
    assert(&ref3 == &__RXX get<InitList>(v));
}
