...
constexpr void swap(
    NotMoveAssignableWithSwap&, NotMoveAssignableWithSwap&) noexcept {}
...
struct NothrowTypeImp {
    ...
    constexpr NothrowTypeImp& operator=(NothrowTypeImp&& o) noexcept(
        NT_MoveAssign) {
        ++*move_assign_called;
...
template <bool NT_Copy, bool NT_Move, bool NT_CopyAssign, bool NT_MoveAssign,
    bool NT_Swap>
constexpr void swap(NothrowTypeImp<NT_Copy, NT_Move, NT_CopyAssign,
                        NT_MoveAssign, NT_Swap, true>& lhs,
    NothrowTypeImp<NT_Copy, NT_Move, NT_CopyAssign, NT_MoveAssign, NT_Swap,
        true>& rhs) noexcept(NT_Swap) {
    ++*lhs.swap_called;
    do_throw<!NT_Swap>();
...
struct NonThrowingNonNoexceptType {
    int value;
    int* move_called;
    constexpr NonThrowingNonNoexceptType(int v, int* mv_called)
        : value(v)
        , move_called(mv_called) {}
    constexpr NonThrowingNonNoexceptType(
        NonThrowingNonNoexceptType&& o) noexcept(false)
        : value(o.value)
        , move_called(o.move_called) {
        ++*move_called;
        o.value = -1;
    }
    NonThrowingNonNoexceptType& operator=(
        NonThrowingNonNoexceptType&&) noexcept(false) {
...
constexpr void test_swap_same_alternative() {
    {
        using V = __RXX variant<ThrowingTypeWithNothrowSwap, int>;
        int move_called = 0;
        int move_assign_called = 0;
        int swap_called = 0;
        V v1(std::in_place_index<0>, 42, &move_called, &move_assign_called,
            &swap_called);
        V v2(std::in_place_index<0>, 100, &move_called, &move_assign_called,
            &swap_called);
        v1.swap(v2);
        assert(swap_called == 1);
        assert(__RXX get<0>(v1).value == 100);
        assert(__RXX get<0>(v2).value == 42);
        swap(v1, v2);
        assert(swap_called == 2);
        assert(__RXX get<0>(v1).value == 42);
        assert(__RXX get<0>(v2).value == 100);

        assert(move_called == 0);
        assert(move_assign_called == 0);
    }
    {
        using V = __RXX variant<NothrowMoveable, int>;
        int move_called = 0;
        int move_assign_called = 0;
        int swap_called = 0;
        V v1(std::in_place_index<0>, 42, &move_called, &move_assign_called,
...
void test_swap_same_alternative_throws() {
#if RXX_WITH_EXCEPTIONS
    {
        using V = __RXX variant<NothrowTypeWithThrowingSwap, int>;
        int move_called = 0;
        int move_assign_called = 0;
        int swap_called = 0;
        V v1(std::in_place_index<0>, 42, &move_called, &move_assign_called,
    ...
    {
        using V = __RXX variant<ThrowingMoveCtor, int>;
        int move_called = 0;
        int move_assign_called = 0;
        int swap_called = 0;
        V v1(std::in_place_index<0>, 42, &move_called, &move_assign_called,
        ...
        assert(move_assign_called == 0);
        assert(swap_called == 0);
        assert(__RXX get<0>(v1).value ==
            42); // throw happened before v1 was moved from
        assert(__RXX get<0>(v2).value == 100);
    }
    {
        using V = __RXX variant<ThrowingMoveAssignNothrowMoveCtor, int>;
        int move_called = 0;
        int move_assign_called = 0;
        int swap_called = 0;
        V v1(std::in_place_index<0>, 42, &move_called, &move_assign_called,
        ...
        assert(swap_called == 0);
        assert(__RXX get<0>(v1).value == -1); // v1 was moved from
        assert(__RXX get<0>(v2).value == 100);
    }
#endif
}

constexpr void test_swap_different_alternatives() {
    {
        using V = __RXX variant<NothrowMoveCtorWithThrowingSwap, int>;
        int move_called = 0;
        int move_assign_called = 0;
        int swap_called = 0;
        V v1(std::in_place_index<0>, 42, &move_called, &move_assign_called,
...
void test_swap_different_alternatives_throws() {
#if RXX_WITH_EXCEPTIONS
    {
        using V = __RXX variant<ThrowingTypeWithNothrowSwap,
            NonThrowingNonNoexceptType>;
        int move_called1 = 0;
        int move_assign_called1 = 0;
        int swap_called1 = 0;
        int move_called2 = 0;
        V v1(std::in_place_index<0>, 42, &move_called1, &move_assign_called1,
            &swap_called1);
        ...
        if (move_called2 != 0)
            ...
            assert(__RXX get<1>(v2).value == 100);
    }
    {
        using V = __RXX variant<NonThrowingNonNoexceptType,
        ...
        V v2(std::in_place_index<1>, 100, &move_called2, &move_assign_called2,
            &swap_called2);
        try {
            v1.swap(v2);
            assert(false);
    ...
    // guarantee.
    {
        using V = __RXX variant<ThrowingTypeWithNothrowSwap, NothrowMoveable>;
        int move_called1 = 0;
        int move_assign_called1 = 0;
        int swap_called1 = 0;
        int move_called2 = 0;
...
constexpr void test_swap_sfinae() {
    {
        // This variant type does not provide either a member or
        // non-member swap but is still swappable via the generic swap
        // algorithm, since the variant is move constructible and move
        // assignable.
        using V = __RXX variant<int, NotSwappable>;
        static_assert(!has_swap_member<V>(), "");
        static_assert(std::is_swappable_v<V>, "");
    }
    {
        using V = __RXX variant<int, NotCopyable>;
        static_assert(!has_swap_member<V>(), "");
        static_assert(!std::is_swappable_v<V>, "");
    }
    {
        using V = __RXX variant<int, NotCopyableWithSwap>;
        static_assert(!has_swap_member<V>(), "");
        static_assert(!std::is_swappable_v<V>, "");
    }
    {
        using V = __RXX variant<int, NotMoveAssignable>;
        static_assert(!has_swap_member<V>(), "");
        static_assert(!std::is_swappable_v<V>, "");
    }
}
...
constexpr void test_swap_noexcept() {
    {
        using V = __RXX variant<int, NothrowMoveable>;
    ...
    {
        using V = __RXX variant<int, NothrowMoveCtor>;
        static_assert(std::is_swappable_v<V> && has_swap_member<V>(), "");
        static_assert(!std::is_nothrow_swappable_v<V>, "");
        // instantiate swap
        V v1, v2;
    ...
    {
        using V = __RXX variant<int, ThrowingTypeWithNothrowSwap>;
        static_assert(std::is_swappable_v<V> && has_swap_member<V>(), "");
        static_assert(!std::is_nothrow_swappable_v<V>, "");
        // instantiate swap
        V v1, v2;
    ...
    {
        using V = __RXX variant<int, ThrowingMoveAssignNothrowMoveCtor>;
        static_assert(std::is_swappable_v<V> && has_swap_member<V>(), "");
        static_assert(!std::is_nothrow_swappable_v<V>, "");
        // instantiate swap
        V v1, v2;
    ...
    {
        using V = __RXX variant<int, ThrowingMoveAssignNothrowMoveCtorWithSwap>;
        static_assert(std::is_swappable_v<V> && has_swap_member<V>(), "");
        static_assert(std::is_nothrow_swappable_v<V>, "");
        // instantiate swap
        V v1, v2;
    ...
    {
        using V = __RXX variant<int, NotMoveAssignableWithSwap>;
        static_assert(std::is_swappable_v<V> && has_swap_member<V>(), "");
        static_assert(std::is_nothrow_swappable_v<V>, "");
        // instantiate swap
        V v1, v2;
...
template class __RXX variant<int, NotSwappable>;
