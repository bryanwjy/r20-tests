...
struct CopyAssign {
    constexpr CopyAssign(int v, int* alv, int* cpy_ctr, int* cpy_assi,
        int* move_ctr, int* move_assi)
        : value(v)
        , alive(alv)
    ...
    constexpr CopyAssign(CopyAssign const& o)
        : value(o.value)
        , alive(o.alive)
    ...
    constexpr CopyAssign& operator=(CopyAssign const& o) {
        value = o.value;
        alive = o.alive;
        copy_construct = o.copy_construct;
        copy_assign = o.copy_assign;
...
struct NTCopyAssign {
    ...
    NTCopyAssign(NTCopyAssign const&) = default;
    NTCopyAssign(NTCopyAssign&&) = default;
    NTCopyAssign& operator=(NTCopyAssign const& that) {
        value = that.value;
        return *this;
    };
...
struct TCopyAssign {
    ...
    TCopyAssign(TCopyAssign const&) = default;
    TCopyAssign(TCopyAssign&&) = default;
    TCopyAssign& operator=(TCopyAssign const&) = default;
...
struct TCopyAssignNTMoveAssign {
    ...
    TCopyAssignNTMoveAssign(TCopyAssignNTMoveAssign const&) = default;
    TCopyAssignNTMoveAssign(TCopyAssignNTMoveAssign&&) = default;
    TCopyAssignNTMoveAssign& operator=(
        TCopyAssignNTMoveAssign const&) = default;
...
struct CopyCannotThrow {
    ...
    CopyCannotThrow(CopyCannotThrow const&) noexcept { ++alive; }
    CopyCannotThrow(CopyCannotThrow&&) noexcept { assert(false); }
    CopyCannotThrow& operator=(CopyCannotThrow const&) noexcept = default;
...
constexpr void test_copy_assignment_not_noexcept() {
    {
        using V = __RXX variant<CopyMaybeThrows>;
        static_assert(!std::is_nothrow_copy_assignable<V>::value, "");
    }
    {
        using V = __RXX variant<int, CopyDoesThrow>;
        static_assert(!std::is_nothrow_copy_assignable<V>::value, "");
    }
}
...
constexpr void test_copy_assignment_sfinae() {
    ...
    {
        using V = __RXX variant<int, NTCopyAssign>;
        static_assert(!std::is_trivially_copy_assignable<V>::value, "");
        static_assert(std::is_copy_assignable<V>::value, "");
    }
    {
        using V = __RXX variant<int, TCopyAssign>;
        static_assert(std::is_trivially_copy_assignable<V>::value, "");
    }
    {
        using V = __RXX variant<int, TCopyAssignNTMoveAssign>;
        static_assert(std::is_trivially_copy_assignable<V>::value, "");
    }
    {
        using V = __RXX variant<int, CopyOnly>;
...
constexpr void test_copy_assignment_same_index() {
    {
    ...
    {
        using V = __RXX variant<int, CopyAssign, unsigned>;
        int alive = 0;
        int copy_construct = 0;
        int copy_assign = 0;
        int move_construct = 0;
        int move_assign = 0;
        V v1(std::in_place_type<CopyAssign>, 43, &alive, &copy_construct,
            &copy_assign, &move_construct, &move_assign);
        V v2(std::in_place_type<CopyAssign>, 42, &alive, &copy_construct,
            &copy_assign, &move_construct, &move_assign);
        V& vref = (v1 = v2);
        assert(&vref == &v1);
        assert(v1.index() == 1);
    ...
    }
    {
        struct {
            constexpr Result<int> operator()() const {
                using V = __RXX variant<int, TCopyAssign, unsigned>;
    ...
    }
    {
        struct {
            constexpr Result<int> operator()() const {
                using V = __RXX variant<int, TCopyAssignNTMoveAssign, unsigned>;
...
constexpr void test_copy_assignment_different_index() {
    {
        using V = __RXX variant<int, long, unsigned>;
        V v1(43);
        V v2(42l);
...
void test_assignment_throw() {
    ...
    // difference indices
    {
        using V = __RXX variant<int, CopyThrows, std::string>;
        ...
        // Test that copy construction is used directly if move construction may
        // throw, resulting in a valueless variant if copy throws.
        assert(v1.valueless_by_exception());
    }
    {
        using V = __RXX variant<int, MoveThrows, std::string>;
        V v1(std::in_place_type<std::string>, "hello");
    ...
    }
    {
        ...
        using V = __RXX variant<int, CopyCannotThrow, std::string>;
        V v1(std::in_place_type<std::string>, "hello");
...
constexpr void test_constexpr_assign_imp(T&& v, ValueType&& new_value) {
    ...
    Variant const cp(std::forward<ValueType>(new_value));
    v = cp;
...
struct NonTrivialCopyAssign {
    ...
    constexpr NonTrivialCopyAssign(NonTrivialCopyAssign const& other)
        : i(other.i) {}
    constexpr NonTrivialCopyAssign& operator=(NonTrivialCopyAssign const& o) {
        i = o.i;
        return *this;
    }
    constexpr ~NonTrivialCopyAssign() = default;
    friend constexpr bool operator==(
        NonTrivialCopyAssign const& x, NonTrivialCopyAssign const& y) {
        return x.i == y.i;
    }
};
...
constexpr void test_constexpr_copy_assignment_non_trivial() {
    // Make sure we properly propagate triviality, which implies constexpr-ness
    // (see P0602R4).
    using V = __RXX variant<long, void*, NonTrivialCopyAssign>;
    static_assert(!std::is_trivially_copyable<V>::value, "");
    static_assert(!std::is_trivially_copy_assignable<V>::value, "");
    test_constexpr_assign_imp<0>(V(42l), 101l);
    test_constexpr_assign_imp<0>(V(nullptr), 101l);
    ...
    test_constexpr_assign_imp<2>(
        V(NonTrivialCopyAssign(3)), NonTrivialCopyAssign(5));
}
