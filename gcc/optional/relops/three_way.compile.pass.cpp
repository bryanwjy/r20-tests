...
void test01() {
    ...
    static_assert(
        !std::three_way_comparable_with<O, __RXX optional<unsigned>>);
}
...
template <typename T>
concept has_spaceship = requires(T const& t) { t <=> t; };

void test03() {
    struct E {
        auto operator<=>(E const&) const { return std::strong_ordering::equal; }
    };
    static_assert(!std::three_way_comparable<E>); // not equality comparable
    using O = __RXX optional<E>;
