...
int main() {
    {
        static_assert(std::is_trivially_destructible<
                          __RXX optional<trivially_destructible>>(),
            "");
    }
...

    {
        ...
        static_assert(std::is_copy_assignable<O>(), "");
        /*
         * T should be move constructible due to [12.8/11], which is a new rule
         * in C++1y not yet implemented by GCC. Because there is already a
         * special exception in C++11 for the generation of the special members
         * that GCC implements (at least some of the time), this does not affect
         * the __RXX optional implementation however. So the assertion for T
         * should be changed (or removed altogether) when the time comes, but
         * the rest should however remain correct and unchanged.
         */
...

    {
        ...
        /*
         * Paragraph 23 of same leads to a similar situation but with respect to
         * move assignment.
         */
...

    {
         ...
         */
        using A = __RXX optional<int&>;
        using B = __RXX optional<int&&>;
        using C1 = __RXX optional<std::in_place_t>;
        using C2 = __RXX optional<std::in_place_t const>;
        using C3 = __RXX optional<std::in_place_t volatile>;
        using C4 = __RXX optional<std::in_place_t const volatile>;
        using D1 = __RXX optional<__RXX nullopt_t>;
        using D2 = __RXX optional<__RXX nullopt_t const>;
        using D3 = __RXX optional<__RXX nullopt_t volatile>;
        using D4 = __RXX optional<__RXX nullopt_t const volatile>;

        using X = std::tuple<A, B, C1, C2, C3, C4, D1, D2, D3, D4>;
    }

    {
        __RXX optional<int const> o{42};
        static_assert(std::is_same<decltype(o)::value_type, int const>(), "");
        assert(o);
        assert(*o == 42);
    }

    {
        constexpr __RXX optional<int const> o{33};
        static_assert(std::is_same<decltype(o)::value_type, int const>(), "");
        static_assert(o, "");
        static_assert(*o == 33, "");
    }
...
struct JustEq {};
bool operator==(JustEq const&, JustEq const&);
...
struct JustLt {};
bool operator<(JustLt const&, JustLt const&);
...
static_assert(
    !std::is_assignable<optional<JustEq>&, optional<JustLt>>::value, "");
static_assert(!std::is_assignable<optional<JustEq>&, JustLt>::value, "");
static_assert(
    !std::is_assignable<optional<JustEq>&, optional<JustLt>&>::value, "");
static_assert(!std::is_assignable<optional<JustEq>&, JustLt&>::value, "");
