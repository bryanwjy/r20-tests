...
struct value_type : private mixin_counter {
    ...
    value_type(value_type const& other) : state(other.state) {
...
int main() {
    ...
    // From engaged optional
    {
        ...
        o = p;
        assert(o && o->state == S::throwing_copy);
        assert(p && p->state == S::throwing_copy);
    }

    {
        ...
        o = std::move(p);
        assert(o && o->state == S::throwing_move);
        assert(p && p->state == S::moved_from);
    }

    {
        ...
        assert(o && o->state == S::threw);
        assert(p && p->state == S::throwing_copy_assignment);
    }
