...
void test01() {
  ...
  static_assert(ranges::common_range<R const>);
}
...
void test05() {
  ...
  auto v = x | rxx::views::join | views::lazy_split(' ');
...
void test06() {
  ...
  auto v = r | views::transform(std::identity{}) | rxx::views::join;
  ...
  static_assert(
      !std::same_as<decltype(ranges::end(v)), decltype(ranges::cend(v))>);
...
void test07() {
  ...
  auto joined = nested_vectors | rxx::views::join | rxx::views::join;
...
void test08() {
  ...
  ranges::single_view<ranges::single_view<X>> s{std::in_place, std::in_place,
                                                5};
...
template <auto join = rxx::views::join> void test09() {
...
void test11() {
  ...
  int r[] = {1, 2, 3};
  auto v = r | views::transform([](int n) {
  ...
  auto w = r | views::transform([](int) { return std::array<S, 2>{}; }) |
           rxx::views::join;
  for (auto &i : w)
    ;
}
...
void test12() {
  ...
  constexpr auto b = [] {
    auto r =
        std::views::iota(0, 5) | std::views::lazy_split(0) | rxx::views::join;
...
void test13() {
  ...
  auto l = std::views::transform([](auto x) {
    return x | std::views::transform([i = 0](auto y) { return y; });
  });
  std::vector<std::vector<int>> v{{5, 6, 7}};
  (void)(v | l | rxx::views::join);
}
