...
template <typename T> struct OutputContainer : public BoundsContainer<T> {
  ...
  OutputContainer(T *first, T *last)
      : BoundsContainer<T>(first, last), incrementedto(first),
 ...
 */
template <class T> struct output_iterator_wrapper {
protected:
...
public:
  ...
  output_iterator_wrapper &operator=(output_iterator_wrapper const &) = default;
  ...
  template <typename U> void operator,(U const &) const = delete;
...
struct forward_iterator_wrapper : public input_iterator_wrapper<T> {
  ...
  forward_iterator_wrapper &
  operator=(forward_iterator_wrapper const &) = default;
...
struct bidirectional_iterator_wrapper : public forward_iterator_wrapper<T> {
  ...
  bidirectional_iterator_wrapper(bidirectional_iterator_wrapper const &) =
      default;

  bidirectional_iterator_wrapper &
  operator=(bidirectional_iterator_wrapper const &) = default;
...
struct random_access_iterator_wrapper
  ...
  random_access_iterator_wrapper(random_access_iterator_wrapper const &) =
      default;

  random_access_iterator_wrapper &
  operator=(random_access_iterator_wrapper const &) = default;
...
struct contiguous_iterator_wrapper : random_access_iterator_wrapper<T> {
  ...
  friend contiguous_iterator_wrapper operator+(contiguous_iterator_wrapper iter,
                                               difference_type n) {
    return iter += n;
  }
  ...
  friend difference_type operator-(contiguous_iterator_wrapper l,
                                   contiguous_iterator_wrapper r) {
...
struct input_iterator_wrapper_nocopy : input_iterator_wrapper<T> {
  ...
  input_iterator_wrapper_nocopy(input_iterator_wrapper_nocopy const &) = delete;
  input_iterator_wrapper_nocopy &
  operator=(input_iterator_wrapper_nocopy const &) = delete;
...
template <typename T, template <typename> class Iter> class test_range {
  ...
  template <typename I> struct sentinel {
    T *end;
...
struct test_sized_range_sized_sent : test_sized_range<T, Iter> {
  ...
  template <typename I> struct sentinel {
    ...
    friend std::iter_difference_t<I> operator-(sentinel const &s,
                                               I const &i) noexcept {
      return std::iter_difference_t<I>(s.end - i.ptr);
    }

    friend std::iter_difference_t<I> operator-(I const &i,
                                               sentinel const &s) noexcept {
      return std::iter_difference_t<I>(i.ptr - s.end);
    }
  };
