...
public:
    ...
    typename std::enable_if<std::is_assignable<T&, U>::value>::type operator=(
        U&& new_val) const {
        ITERATOR_assert(SharedInfo->writtento[ptr - SharedInfo->first] == 0);
...
public:
    ...
    output_iterator_wrapper(T* ptr, ContainerType* info)
        : ptr(ptr)
        , SharedInfo(info) {
        ITERATOR_assert(ptr >= SharedInfo->first && ptr <= SharedInfo->last);
    }
    ...
    WritableObject<T> operator*() const {
        ITERATOR_assert(ptr < SharedInfo->last);
        ITERATOR_assert(
    ...
    output_iterator_wrapper& operator++() {
        ITERATOR_assert(SharedInfo && ptr < SharedInfo->last);
        ITERATOR_assert(ptr >= SharedInfo->incrementedto);
        ptr++;
...
public:
    ...
    bool operator==(input_iterator_wrapper const& in) const {
        ITERATOR_assert(SharedInfo && SharedInfo == in.SharedInfo);
        ITERATOR_assert(
    ...
    T& operator*() const {
        ITERATOR_assert(SharedInfo && ptr < SharedInfo->last);
        ITERATOR_assert(ptr >= SharedInfo->first);
        return *ptr;
    }
...
struct forward_iterator_wrapper : public input_iterator_wrapper<T> {
    ...
    T& operator*() const {
        ITERATOR_assert(this->SharedInfo && this->ptr < this->SharedInfo->last);
        return *(this->ptr);
    }
...
struct random_access_iterator_wrapper :
    ...
    random_access_iterator_wrapper& operator+=(ptrdiff_t n) {
        if (n > 0) {
            ITERATOR_assert(n <= this->SharedInfo->last - this->ptr);
            this->ptr += n;
        } else {
            ITERATOR_assert(-n <= this->ptr - this->SharedInfo->first);
            this->ptr += n;
        }
        return *this;
    }
    ...
    ptrdiff_t operator-(random_access_iterator_wrapper<T> const& in) const {
        ITERATOR_assert(this->SharedInfo == in.SharedInfo);
...
struct test_container {
    ...
    ItType<T> it(int pos) {
        ITERATOR_assert(pos >= 0 && (unsigned)pos <= size());
        return ItType<T>(bounds.first + pos, &bounds);
    }

    ItType<T> it(T* pos) {
        ITERATOR_assert(pos >= bounds.first && pos <= bounds.last);
